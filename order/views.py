...
def shopcart(request):
    ...
    shopcart = ShopCart.objects.filter(user_id=current_user.id, product__isnull=False, product__price__isnull=False)
    total = shopcart.annotate(item_total=F('product__price') * F('quantity')).aggregate(total_price=models.Sum('item_total'))['total_price'] or 0
    context = {
        'shopcart': shopcart,
        'category': category,
        'total': total,
